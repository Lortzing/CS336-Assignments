...
def train_bpe(
    ...
    if PARALLEL:
        with Pool(processes=cpu_count()) as pool:
            for partial_counter in tqdm(
                pool.imap_unordered(lambda args: pre_tokenization_with_chunk(*args), args_list),
                total=len(args_list),
        ...
        for args in tqdm(args_list, desc="Pre-tokenizing chunks"):
            counter.update(pre_tokenization_with_chunk(*args))
...
if __name__ == "__main__":
    train_bpe("/home/lu0qlng/cs336/assignment1-basics/tests/fixtures/tinystories_sample.txt", 1000, ["<|endoftext|"])
    pass
