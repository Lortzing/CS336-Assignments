...
def train_bpe(
    ...
    for _ in range(target_merges):
        ...
        top_pair, top_count = pair_freq.most_common(1)[0]
        if top_count <= 0:
            break  # no meaningful pairs (frequency zero)
        merges.append(top_pair)
        # Create the new merged token (bytes concatenation of the pair)
        new_token = top_pair[0] + top_pair[1]
        vocab[len(vocab)] = new_token  # assign a new ID for the merged token
        
        # Get all words that contain this pair (to update them)
        words_with_pair = pair_to_words.pop(top_pair, set())
        # Remove this pair from frequency dictionary (it will be replaced by new pairs)
        del pair_freq[top_pair]
        
        # Process each affected word that contained the pair
        for word in list(words_with_pair):
            if word not in words_counter:
                # Word might have been updated already in this iteration
                continue
            ...
            i = 0
            while i < len(word):
                # If we find the target pair at position i, merge it into new_token
            ...
            # Update word frequency dictionary: remove old word, add new (merged) word
            words_counter.pop(word)
            words_counter[new_word] = words_counter.get(new_word, 0) + word_count
            
            # Compute pair occurrence counts in the old and new word for frequency updates
            ...
            # Decrement frequencies for pairs that belonged to the old word
            for pair, occ in old_pairs.items():
                pair_freq[pair] -= occ * word_count
                if pair in pair_to_words:
                    pair_to_words[pair].discard(word)
                    if not pair_to_words[pair]:
                        # Clean up pair with no occurrences left
                        pair_to_words.pop(pair)
            
            # Increment frequencies for pairs in the new word
