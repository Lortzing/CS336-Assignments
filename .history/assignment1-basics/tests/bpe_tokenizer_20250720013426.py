...
def pre_tokenization(start: int, end: int, input_path: str|os.PathLike, special_tokens: List[str]) -> Dict[Tuple[bytes, ...], int]:
    counter = Counter()
...
def train_bpe(
    ...
    for start, end in zip(chunks[:-1], chunks[1:]):
        counter_chunk = pre_tokenization(start, end, input_path, special_tokens)
        counter.update(counter_chunk)
