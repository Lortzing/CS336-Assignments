...
def pre_tokenization_with_text(text: str, special_tokens: List[str]) -> Counter[tuple[bytes]]:
    ...
    if not text:
        return counter
    # Split text to isolate special tokens as separate parts
    if special_tokens:
            ...
            while prt < len(word):
                if (
                    ...
                    if pre:
                        ...
                        pairs_frequency[(pre, word[prt])] -= num

                    if prt < len(word) - 2:
                        new_pair_suf = (new_token, word[prt + 2])
                        pairs_frequency[new_pair_suf] += num
                        new_pairs.append((new_token, word[prt + 2]))
                        pairs_frequency[(word[prt + 1], word[prt + 2])] -= num
