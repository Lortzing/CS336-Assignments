...
def train_bpe(
    ...
    pairs_frequency: Counter[tuple[tuple[int, ...], tuple[int, ...], tuple[int, ...]]] = Counter()
    ...
    for word in words_counter:
        ...
        for i in range(len(word) - 1):
            pair_0, pair_1 = tuple(-b for b in word[i]), tuple(-b for b in word[i + 1])
            
            pair: tuple[tuple[int], tuple[int], tuple[int]] = (pair_0 + pair_1, pair_0, pair_1)
            pairs_frequency[pair] -= words_counter[word]
            pair_bytes = word[i: i + 2]
            
            if pair_bytes not in words_pairs_map[word]:
                words_pairs_map[word].append(pair_bytes)
            if pair_bytes in pairs_words_map:
                if word not in pairs_words_map[pair_bytes]:
                    pairs_words_map[pair_bytes].append(word)
            else:
                pairs_words_map[pair_bytes] = [word]
    ...
    for i in range(totel_loop):        
        ...
        pair_bytes = (
            ...
            bytes(tuple(-b for b in top_pair[2]))
        )
        ...
        for word in pairs_words_map.pop(pair_bytes, []):
            ...
            while prt < len(word):
                if (
                    prt < len(word) - 1
                    and word[prt] == pair_bytes[0]
                    and word[prt + 1] == pair_bytes[1]
                ):
                    if pre:
                        new_pair_pre_0, new_pair_pre_1 = (
                            tuple(-b for b in pre),
                            tuple(-b for b in new_token)
                        )
                        new_pair_pre = (
                            new_pair_pre_0+new_pair_pre_1,
                            new_pair_pre_0,
                        ...
                        old_pair_pre_0, old_pair_pre_1 = (
                            tuple(-b for b in pre),
                            top_pair[1]
                        )
                        
                        old_pair_pre = (old_pair_pre_0+old_pair_pre_1, old_pair_pre_0 , old_pair_pre_1)
                        pairs_frequency[old_pair_pre] += num

                    if prt < len(word) - 2:
                        new_pair_suf_0, new_pair_suf_1 = (
                            tuple(-b for b in new_token),
                            tuple(-b for b in word[prt + 2])
                        )
                        new_pair_suf = (
                            new_pair_suf_0+new_pair_suf_1,
                            new_pair_suf_0,
                        ...
                        old_pair_suf_0, old_pair_suf_1 = (
                            top_pair[2],
                            tuple(-b for b in word[prt + 2])
                        )

                        old_pair_suf = (old_pair_suf_0+old_pair_suf_1, old_pair_suf_0, old_pair_suf_1)
                        pairs_frequency[old_pair_suf] += num
            ...
            for h in words_pairs_map[word_new_tuple]:
                pairs_words_map[h].remove(word)
                pairs_words_map[h].append(word_new_tuple)
