...
def train_bpe(
    ...
    pairs_frequency: Counter[tuple[tuple[bytes], tuple[bytes]]] = Counter()
    ...
    for word in words_counter:
        ...
        for i in range(len(word) - 1):
            pair: tuple[tuple[bytes], tuple[bytes]] = (word[:i], word[i + 1:])
    ...
    for i in range(totel_loop):
        ...
        top_pair = heapq.nlargest(1, pairs_frequency.items(), key=lambda x: x[1])[0]
