...
def pre_tokenization_with_chunk(start: int, end: int, input_path: str | os.PathLike, special_tokens: List[str]) -> Counter[tuple[bytes]]:
...
def pre_tokenization_with_text(text: str, special_tokens: List[str]) -> Counter[tuple[bytes]]:
    ...
    print(list(re.splititer(f"({escape})", text)))
...
def train_bpe(
    ...
    for start, end in zip(chunks[:-1], chunks[1:]):
        counter.update(pre_tokenization_with_chunk(start, end, input_path, special_tokens))
...
if __name__ == "__main__":
    ...
    print(pre_tokenization_with_text(chunk, special_tokens))
    pass
