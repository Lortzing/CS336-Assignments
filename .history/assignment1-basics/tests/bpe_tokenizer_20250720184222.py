...
def find_chunk_boundaries(file: BinaryIO, desired_num_chunks: int, split_special_token: bytes) -> list[int]:
    ...
    for bi in tqdm(ange(1, len(chunk_boundaries) - 1)):
...
def train_bpe(
    ...
    with open(input_path, "rb") as f:
        chunks = find_chunk_boundaries(f, 1000, b"<|endoftext|>")
    ...
    for _ in tqdm(range(target_merges), desc="Merging pairs"):
        if not pair_freq:
...
if __name__ == "__main__":
    input_path = "/home/lu0qlng/cs336/assignment1-basics/data/TinyStoriesV2-GPT4-train.txt"
    ...
    def save_bpe_json(vocab: dict[int, bytes], merges: list[tuple[bytes, bytes]], out_path: str):
        ...
        with open(out_path+"vocab_tiny.json", "w", encoding="utf-8") as f:
            json.dump(vocab_serializable, f, indent=2)
            
        with open(out_path+"merges_tiny.json", "w", encoding="utf-8") as f:
