...
def train_bpe(
    ...
    for i in range(totel_loop):
        print(pairs_frequency_list)
        if not pairs_frequency_list:
            break

        frequency, top_pair = heapq.heappop(pairs_frequency_list)
        ...
        for word in pairs_words_map.pop(pair_bytes, []):
            ...
            while prt < len(word) - 1:
                if word[prt] == pair_bytes[0] and word[prt + 1] == pair_bytes[1]:
                    if pre:
                        ...
                        new_pairs.append(new_pair_pre)
                        pairs_frequency[(tuple(-b for b in pre), top_pair[0])] += 1
                        
                    if prt < len(word) - 2:
                        new_pair_suf: tuple[tuple[int, ...], tuple[int, ...]] = (tuple(-b for b in new_token), tuple(-b for b in word[prt + 2]))
                        new_pair_frequency[new_pair_suf] -= 1
                        new_pairs.append(new_pair_suf)
                        pairs_frequency[(top_pair[1], tuple(-b for b in word[prt + 2]))] += 1
            ...
            for new_pair in new_pairs:
                if new_pair in pairs_words_map:
                    pairs_words_map[new_pair].append(word_new)
                else:
                    pairs_words_map[new_pair] = [word_new]

        for new_pair, frequency in new_pair_frequency.items():
            heapq.heappush(pairs_frequency_list, (frequency, new_pair))
    return vocab, merges
