...
def train_bpe(
    ...
    for word in words_counter:
        ...
        for i in range(len(word) - 1):
            pair: tuple[bytes] = tuple(bytes(-b for b in token) for token in word[i : i + 2])
...
if __name__ == "__main__":
    ...
    word = tuple(bytes([b]) for b in "你好".encode("utf-8"))
    
    print(word)
    s = tuple(tuple(-b for b in token) for token in word[1 : 1 + 2])
    print(tuple(tuple(-b for b in token) for token in word[1 : 1 + 2]))
