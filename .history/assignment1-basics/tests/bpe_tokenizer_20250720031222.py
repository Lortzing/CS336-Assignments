...
def train_bpe(
    ...
    pairs_frequency: Counter[tuple[bytes]] = Counter()
    ...
    for word in words_counter:
        ...
        for i in range(len(word) - 1):
            pair = tuple(-b for token in word[i : i + 2] for b in token)
    ...
    pairs_frequency.update(Counter(pairs_words_map))
    heapq.heapify([(frequency, pair) for pair, frequency in pairs_frequency.items()])

    print(heapq)
    totel_loop = vocab_size - len(vocab)
    # for i in range(totel_loop):
    #     pair, frequency = pairs_frequency.most_common(1)[0]
    #     vocab[len(vocab)] = pair[0] + pair[1]
    #     pairs_frequency -= Counter(pairs_words_map[pair])
