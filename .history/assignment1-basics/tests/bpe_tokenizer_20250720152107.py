...
def train_bpe(
    ...
    pairs_frequency: Counter[tuple[bytes, bytes]] = Counter()
    ...
    for i in range(totel_loop):
        ...
        top_pair: tuple[bytes, bytes] = heapq.nlargest(1, pairs_frequency.items(), key=lambda x: x[1])[0][0]
        ...
        for word in pairs_words_map.pop(top_pair, []):
            words_pairs_map[word].remove(top_pair)
