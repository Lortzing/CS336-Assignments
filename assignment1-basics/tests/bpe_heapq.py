...
def train_bpe(
    ...
    for _ in range(target_merges):
        ...
        while heap:
            ...
            if candidate in pair_freq and -neg_freq == pair_freq[candidate]:
                top_pair = candidate
                break
        else:
            break  # 没有可用的 pair
        ...
        for word in list(words_with_pair):
            ...
            words_counter[new_word] = words_counter.get(new_word, 0) + count

            # update pair_freq and pair_to_words
            old_pairs = Counter((word[i], word[i+1]) for i in range(len(word) - 1))
            new_pairs = Counter((new_word[i], new_word[i+1]) for i in range(len(new_word) - 1))

            for pair, occ in old_pairs.items():
                pair_freq[pair] -= occ * count
                if pair_freq[pair] <= 0:
                    pair_freq.pop(pair)
                else:
